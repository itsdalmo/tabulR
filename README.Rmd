...
tabulR includes just two functions `qtable()` and `qtable_()`, which are meant to 
generate a "quick" table using [data.table](https://github.com/Rdatatable/data.table), 
and aims to balance information and readability in the output. `qtable()` is meant to 
be used with dplyr and it's NSE select statements (but uses the same underlying code as `qtable_()`).
...
Sort of a NSE version of `qtable`. It requires dplyr's, and expects unquoted arguments
for `var`, while allows us to use `one_of()`, `starts_with()` etc:
