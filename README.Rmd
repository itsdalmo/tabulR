...
tabulR includes just two functions `qtable()` and `dtable()`, which are meant to 
generate a "quick" table using [data.table](https://github.com/Rdatatable/data.table), 
and aims to balance information and readability in the output. `dtable` is meant to be
used with dplyr (but uses the same code as qtable).
...
This is not really a NSE version of `qtable`. It also uses dplyr's `select_` to
subset the data, so that we can use e.g. `one_of()` - hence it was given a separate name.
